      ...
      <div className="relative z-10 grid grid-cols-4 h-full">
        {steps.map((step, index) => (
          <div
            key={index}
            className={clsx(
              "flex flex-col justify-end items-center text-white p-6 cursor-pointer transition-all duration-300 border border-white/20 relative",
              "hover:bg-black/40"
            )}
            onMouseEnter={() => {
              setBgImage(step.image);
          ...
          >
            {/* Step Number */}
            <h2 className="text-5xl font-bold opacity-60">{step.id}</h2>

            {/* Title */}
            <p className="text-lg mt-2 text-center opacity-80">{step.title}</p>

            {/* Animated Description (Shown Only When Hovering Over Box) */}
            <div
              className={clsx(
                "absolute bottom-0 left-0 w-full bg-black/80 p-4 text-sm text-center transition-all duration-300 opacity-0 transform translate-y-full",
                hoverIndex === index ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"
              )}
            >
              {step.description}
